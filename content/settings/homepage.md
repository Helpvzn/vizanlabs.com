---
heroHeadline: Level Up Your Career with Premium Templates
heroSubheadline: Professionally designed resume templates, Excel sheets, eco-friendly planners, and productivity tools. Instant download, lifetime access.
headlineColor: '#ffffff'
subheadlineColor: '#999999'
heroCta: Browse Templates →
heroCtaLink: /shop
showStats: true
stats:
  - number: 5000+
    label: Customers
  - number: 100+
    label: Templates
  - number: 4.9★
    label: Rating
  - number: 24h
    label: Support
categoriesTitle: Browse by Category
categoriesTagline: Find exactly what you need
categories:
  - name: AI Prompts
    icon: ✨
    href: /shop?category=AI+Prompts
    desc: Power your workflow with AI
  - name: Excel Templates
    icon: /uploads/excel.png
    href: /shop?category=Excel+Templates
    desc: Business, finance & tracking
  - name: Eco Planners
    icon: 🌿
    href: /shop?category=Eco+Planners
    desc: Sustainable life planning
  - name: Habit Trackers
    icon: /uploads/habit.png
    href: /shop?category=Habit+Trackers
    desc: Build better routines
  - name: Business Kits
    icon: 💼
    href: /shop?category=Business+Kits
    desc: Complete business packs
customizeEnabled: true
customizeTitle: Need a Custom Template?
customizeSubtitle: Get your resume or spreadsheet customized by our experts. Professional, fast, and affordable.
customizePrice: $2.99
customizeCtaText: Get Customized Now →
customizeCtaLink: mailto:support@vizanlabs.com?subject=Custom%20Request&body=Hi,%20I%20want%20a%20custom%20template
customizeServices:
  - icon: /uploads/premium.png
    title: Custom Resume Design
    desc: We customize any resume template with your details and branding
  - icon: /uploads/excel.png
    title: Excel Sheet Setup
    desc: We configure your Excel template with your data and formulas
  - icon: ⚡
    title: Fast Delivery
    desc: Get your customized file within 24 hours via email
featuresTitle: Why Choose Us?
features:
  - icon: ⚡
    title: Instant Download
    desc: Get your files immediately after purchase
  - icon: ♾️
    title: Lifetime Access
    desc: Download anytime, forever
  - icon: 🔄
    title: Free Updates
    desc: All future improvements included
  - icon: /uploads/vizan variya.png
    title: Support
    desc: We are always here to help
testimonials:
  - name: Vizan Patel
    role: Software Engineer
    text: Got my dream job using the ATS resume template. The formatting was perfect!
    stars: 5
  - name: Rahul Gupta
    role: Business Owner
    text: Excel templates saved me hours every week. Super professional.
    stars: 5
  - name: Ananya Singh
    role: Freelancer
    text: Eco planner helped me organise my entire life. Beautiful design!
    stars: 5
seoTitle: ''
seoDesc: ''
---
